...
const installNode = async (conn, linuxEnv) => {
  try {
    if (linuxEnv === "REDHAT") {
      const { status } = await execRemote(conn, "rpm -ivh /nodejs.rpm");
      return status;
...
const installNginx = async (conn, linuxEnv) => {
  try {
    if (linuxEnv === "REDHAT") {
      const { status } = await execRemote(conn, "rpm -ivh /nginx.rpm");
      return status;
    }
  } catch (e) {
    console.log(e);
...
const installMysql = async (conn, linuxEnv) => {
  try {
    if (linuxEnv === "REDHAT") {
      const { status } = await execRemote(conn, "yum install -y numactl");
      if (status === 0) {
        return 0;
      } else {
        const { status, data } = await execRemote(
          conn,
        ...
        if (status === 0) {
          ...
          const { status } = await execRemote(conn, `yum remove -y ${data}`);
          if (status === 0) {
            return 0;
          } else {
            await execRemote(conn, "rpm -ivh /mysql-common.rpm");
            await execRemote(conn, "rpm -ivh /mysql-libs.rpm");
            await execRemote(conn, "rpm -ivh /mysql-client.rpm");
            await execRemote(conn, "rpm -ivh /mysql-server.rpm");
